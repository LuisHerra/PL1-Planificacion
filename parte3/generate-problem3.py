...
def main():
    ...
    with open(problem_name + ".pddl", 'w') as f:
        ...
        for x in person:
            ...
            f.write("\t(person-available " + x + ")\n")

        # Generar puntos (x, y) aleatorios para cada localizacion
        points = {}
        for x in location:
            points[x] = (random.randint(0, 100), random.randint(0, 100))

        # Escribir costes de vuelo entre todas las localizaciones (para asegurar distancias largas tengan coste alto)
        # Comparativamente alto con coger un peso que es coste 1.
        for l1 in location:
            for l2 in location:
                if l1 != l2:
                    dist = math.hypot(points[l1][0] - points[l2][0], points[l1][1] - points[l2][1])
                    coste = max(2, int(round(dist)))
                    f.write("\t(= (fly-cost " + l1 + " " + l2 + ") " + str(coste) + ")\n")
